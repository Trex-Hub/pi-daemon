...
const gatewayHome = (): string => process.env.GATEWAY_HOME || trueHomedir();
...
export const defaultState = (): GatewayState => {
  return {
    config: {
      discordToken: "",
      adminUserId: "",
...
export const loadState = async (path: string = DEFAULT_STATE_PATH): Promise<GatewayState> => {
  try {
 ...
 */
export const migrateState = async (path: string = DEFAULT_STATE_PATH): Promise<void> => {
  let raw: string;
  try {
...
export const saveState = async (state: GatewayState, path: string = DEFAULT_STATE_PATH): Promise<void> => {
